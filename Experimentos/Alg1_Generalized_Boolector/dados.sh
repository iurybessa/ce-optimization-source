...
do
	...
	do
		...
		{ time ./esbmc_3.1 'min_'${fun[i]} --boolector\
		> log.txt;
		} 2>> log.txt
		...
		x_1=$(head -1 output.txt)
		x_2=$(tail -1 output.txt)
		./valor_fun
		f_i=$(tail -1 output.txt)
		sed -i 11s/.*/'	'${var[i]}' X_prev[2] = {'$x_1','$x_2'};'/ 'min_'${fun[i]}
		sed -i 9s/.*/'	'${var[i]}' f_i = '$f_i-0.0001';'/ 'min_'${fun[i]}
	done
	check=0
	p=$(($p*10))
	sed -i 1s/.*/'#define p '$p/ 'min_'${fun[i]}
